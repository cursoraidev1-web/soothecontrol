...
export default function T6Team({
  section,
  sectionIndex,
}: {
  ...
  const subtitle =
    section.subtitle ||
    "A small, focused team committed to quality, clarity, and fast delivery.";
  ...
  return (
    <section className="t6-section">
      <div className="t6-container">
        <span className="t6-eyebrow">Team</span>
        <EditableText
          as="h2"
          className="t6-title"
          value={title}
        ...
        <div className="t6-bento" style={{ marginTop: 18 }}>
          {filled.map((m, idx) => (
            <div key={`${m.name}-${idx}`} className="t6-card t6-item" style={{ gridColumn: "span 4", padding: 18 }}>
              <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                {m.photoUrl ? (
                  ...
                  <img
                    src={m.photoUrl}
                    alt={m.name}
                    style={{ width: 56, height: 56, borderRadius: 18, objectFit: "cover", border: "1px solid rgba(255,255,255,0.12)" }}
                  />
                ) : (
                  <div className="t6-chip" style={{ width: 56, height: 56, borderRadius: 18 }}>
                    <span style={{ fontWeight: 950 }}>{initials(m.name)}</span>
                  </div>
                )}
                <div style={{ minWidth: 0 }}>
                  ...
                  <EditableText
                    ...
                    value={m.role}
                    placeholder="Role"
                    style={{ color: "var(--t6-muted)", fontWeight: 800, fontSize: 13, marginTop: 2 }}
                    onCommit={(next) => {
                      if (!editor || sectionIndex == null) return;
                      const nextMembers = filled.map((x) => ({ ...x }));
