"use client";

import type { ContactCardSection as ContactCardSectionType } from "@/lib/pageSchema";
import { getLoremSentence } from "@/lib/loremIpsum";

interface ContactCardSectionProps {
  section: ContactCardSectionType;
  sectionIndex?: number;
  profile?: {
    business_name?: string;
    address?: string | null;
    phone?: string | null;
    email?: string | null;
    whatsapp?: string | null;
  };
}

export default function ContactCardSection({
  section,
  sectionIndex: _sectionIndex,
  profile,
}: ContactCardSectionProps) {
  const businessName = profile?.business_name || "Our Business";
  const address = profile?.address || getLoremSentence();
  const phone = profile?.phone || "+1 (555) 123-4567";
  const email = profile?.email || "contact@example.com";
  const whatsapp = profile?.whatsapp;
  const mapEmbedUrl = (() => {
    const link = section.mapLink?.trim();
    if (link && link.includes("output=embed")) return link;
    const query = encodeURIComponent(profile?.address || businessName || "Location");
    return `https://maps.google.com/maps?q=${query}&t=&z=13&ie=UTF8&iwloc=&output=embed`;
  })();

  return (
    <section className="t1-section">
      <div className="t1-container">
        <div className="t1-contact-grid">
          {/* Contact Information */}
          <div className="t1-contact-card">
            <h2 className="t1-section-subheading">Get in Touch</h2>
            <p style={{ marginBottom: "var(--spacing-lg)", color: "var(--color-text-secondary)" }}>
              We'd love to hear from you. Send us a message and we'll respond as soon as possible.
            </p>
            <div className="t1-contact-info-item">
              <svg
                className="t1-contact-info-icon"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth="2"
                  d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"
                />
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth="2"
                  d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"
                />
              </svg>
              <span>{address}</span>
            </div>
            {phone && (
              <div className="t1-contact-info-item">
                <svg
                  className="t1-contact-info-icon"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    strokeWidth="2"
                    d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"
                  />
                </svg>
                <a href={`tel:${phone}`} style={{ color: "var(--color-primary)" }}>
                  {phone}
                </a>
              </div>
            )}
            {email && (
              <div className="t1-contact-info-item">
                <svg
                  className="t1-contact-info-icon"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    strokeWidth="2"
                    d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"
                  />
                </svg>
                <a href={`mailto:${email}`} style={{ color: "var(--color-primary)" }}>
                  {email}
                </a>
              </div>
            )}
            {whatsapp && (
              <div className="t1-contact-info-item">
                <svg
                  className="t1-contact-info-icon"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                >
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" />
                </svg>
                <a
                  href={`https://wa.me/${whatsapp.replace(/\D/g, "")}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  style={{ color: "var(--color-primary)" }}
                >
                  {whatsapp}
                </a>
              </div>
            )}
            {section.mapLink && (
              <div style={{ marginTop: "var(--spacing-md)" }}>
                <a
                  href={section.mapLink}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="t1-btn t1-btn-primary"
                >
                  View on Map
                </a>
              </div>
            )}
          </div>

          {/* Contact Form */}
          {section.showForm && (
            <div className="t1-contact-card">
              <h2 className="t1-section-subheading">Send us a message</h2>
              <form 
                className="t1-contact-form"
                onSubmit={(e) => {
                  e.preventDefault();
                  // Form submission can be handled via email service or API
                  alert("Form submission is not yet configured. Please contact us directly via email or phone.");
                }}
              >
                <div className="t1-form-group">
                  <label htmlFor="name" className="t1-form-label">
                    Name
                  </label>
                  <input
                    type="text"
                    id="name"
                    name="name"
                    required
                    className="t1-form-input"
                  />
                </div>
                <div className="t1-form-group">
                  <label htmlFor="email" className="t1-form-label">
                    Email
                  </label>
                  <input
                    type="email"
                    id="email"
                    name="email"
                    required
                    className="t1-form-input"
                  />
                </div>
                <div className="t1-form-group">
                  <label htmlFor="phone" className="t1-form-label">
                    Phone (Optional)
                  </label>
                  <input
                    type="tel"
                    id="phone"
                    name="phone"
                    className="t1-form-input"
                  />
                </div>
                <div className="t1-form-group">
                  <label htmlFor="subject" className="t1-form-label">
                    Subject
                  </label>
                  <input
                    type="text"
                    id="subject"
                    name="subject"
                    className="t1-form-input"
                  />
                </div>
                <div className="t1-form-group">
                  <label htmlFor="message" className="t1-form-label">
                    Message
                  </label>
                  <textarea
                    id="message"
                    name="message"
                    required
                    className="t1-form-textarea"
                  />
                </div>
                <button type="submit" className="t1-form-submit">
                  Send Message
                </button>
              </form>
            </div>
          )}
        </div>

        {/* Embedded map (adds the "professional" contact feel) */}
        <div
          style={{
            marginTop: "var(--spacing-3xl)",
            borderRadius: "var(--radius-2xl)",
            overflow: "hidden",
            border: "1px solid var(--color-border-light)",
            boxShadow: "var(--shadow-lg)",
            background: "var(--color-bg-light)",
          }}
        >
          <div style={{ position: "relative", width: "100%", aspectRatio: "16 / 9" as any }}>
            <iframe
              title="Map"
              src={mapEmbedUrl}
              style={{ position: "absolute", inset: 0, width: "100%", height: "100%", border: 0 }}
              loading="lazy"
              referrerPolicy="no-referrer-when-downgrade"
            />
          </div>
        </div>
      </div>
    </section>
  );
}
